...
| **Implementadas notificaciones proactivas (event-driven)**: trigger por **DynamoDB Stream** sobre `business` → Lambda `flight_cancellation_detector` → SNS `flight-events` → SQS `proactive-notifications` → Lambda → fan-out de emails vía SNS `notifications` | Completa la feature de TP1. Ops solo cambia `estado_vuelo=CANCELADO` en el master row del vuelo (consola DynamoDB o dashboard interno) y el Stream propaga. El script `scripts/cancel_flight.py` queda como tool de testing local. Ver justificación #28. |
