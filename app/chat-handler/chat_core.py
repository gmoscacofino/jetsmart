...
# ── Auth ──────────────────────────────────────────────────────────────────────
#
# La validación del JWT de Cognito (firma RS256 contra el JWKS, issuer, exp) la
# hace el server FastAPI (server.py) ANTES de llamar a estas funciones. Acá solo
# recibimos el dict de `claims` ya validado y lo usamos como identidad.
...
# ── Resultado de ruta ─────────────────────────────────────────────────────────
#
# Las funciones de ruta devuelven una tupla nativa (status_code, payload_dict).
# server.py la traduce a una Response HTTP (status, JSON body, headers CORS).
# Sin acoplamiento al formato Lambda-proxy: este módulo no conoce HTTP.
...
def handle_chat(user: dict, body: dict) -> tuple:
...
def handle_reservations(user: dict) -> tuple:
    user_id = _user_id(user)
    resp    = biz_table.query(
        KeyConditionExpression="PK = :pk AND begins_with(SK, :prefix)",
        ExpressionAttributeValues={
...
def handle_payment(user: dict, body: dict) -> tuple:
    reservation = body or {}
...
# El routing HTTP (método/path → handle_chat/handle_reservations/handle_payment),
# el preflight OPTIONS, /health y la validación del JWT viven ahora en server.py.
# Este módulo expone solo lógica de negocio nativa, sin conocer HTTP ni Lambda.
